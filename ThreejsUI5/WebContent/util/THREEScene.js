sap.ui.define(
  ["sap/ui/core/Control", "sap/ui/core/HTML"],
    ...
    return Control.extend("influenz.de.threeui5.util.THREEScene", {
      ...
      onBeforeRendering() {
        ...
        jQuery.each(this.getObjects(), (x, object) =>
          objLoader.load(object.getObjFilePath(), loadedObject => {
            ...
            this.getScene().add(
              this.getAggregation("directionalLight").getDirectionalLight()
            );
            this.getScene().add(
              this.getAggregation("ambientLight").getAmbientLight()
            );
          })
        );
      },
      ...
      renderer: {
        render(rendererManager, control) {
          ...
          rendererManager.renderControl(
            control.getAggregation("directionalLight")
          );
          rendererManager.renderControl(
            new HTML().setDOMContent(control.getWebGLRenderer().domElement)
          );
          rendererManager.write("</div>");
